...
# Plots the rms (Through time) of the density (|psi|^2) data 
...
t_vals = data1[:, 0]
rms_vals = np.array([
    np.sqrt(np.mean(data1[i, 1:]**2))
    for i in range(plotnum)
])

# Plotting the graph
fig, ax = plt.subplots(figsize=(6, 6))
ax.set_title(r"RMS of $|\psi|^2$")
ax.set_xlabel(r"$\Gamma t$", fontsize=14)
ax.set_ylabel(r"$RMS[|\psi|^2]$", fontsize=14)
ax.plot(t_vals, rms_vals)
plt.show()
