...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Plot the first-harmonic Fourier amplitude over time."
    )
    parser.add_argument(
        "-f",
        "--filename",
...
def main():
    ...
    ax.set_title(
        f"1st harmonic amplitude vs time (index {args.frame_index})", fontsize=14
    )
    ax.set_ylim(bottom=0)
    fig.tight_layout()

    out_base = f"first_harmonic_amp_vs_time_idx{args.frame_index}"
