...
from simulation_input import load_simulation_input
...
def read_input_parameters(sim_name: str, input_root: str) -> dict[str, float]:
    ...
    sim_input = load_simulation_input(input_path)
    params = {
        "nodes_per_dim": int(sim_input.nodes_per_dim),
        "maxt": float(sim_input.maxt),
        "ht": float(sim_input.ht),
        "width_psi": float(sim_input.width_psi),
        "p0": float(sim_input.p0),
        "Delta": float(sim_input.Delta),
        "omega_r": float(sim_input.omega_r),
        "b0": float(sim_input.b0),
        "num_crit": float(sim_input.num_crit),
        "R": float(sim_input.R),
        "plotnum": int(sim_input.plotnum),
        "seed_amp": float(sim_input.seed_amp),
        "optical_turnoff_time": float(sim_input.optical_turnoff_time),
    }
    return params
